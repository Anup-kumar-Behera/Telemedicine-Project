...
sequelize
  ...
  .catch(err => console.log('BTW, did you enter wrong database credentials for Apponintment?',err))
...
//   console.log(patient.id)
  ...
  }).then(appointments => { console.log("appointments is ",appointments);getResponse(appointments,res)}).catch(err => res.send({status:0,err:err}))
})
...
app.get('/doctor/appointments/:doctorEmail', async (req,res) => {
  ...
  await  Doctor.findOne({where:{email: doctorEmail}}).then(doctor => {
    if(!doctor)     
        did = 0
    else
        did=doctor.id;
    ...
    await Appointment.findAll(
      ...
      console.log(appointments);getResponseDoctor(appointments,res)
...
app.post('/bookappointment/:patientEmail/:doctorEmail', async (req,res) => {
  ...
  let patient,did;
  await  User.findOne({where:{email: patientEmail}}).then(p => {patient=p;
  })
  ...
  // }).catch(err => res.send({status:0,err:err}))
  await Appointment.create({doctorId:did,patientId:patient.id,status:status,language:language}).then(appointment => {
    ...
    io.emit("appointment-created",doctorEmail,newAppointment)
    res.send({status:1,message:'appointment created',appointment:newAppointment})
...
app.post('/patient/Payment/:id',async(req,res)=>{
  const id = req.params.id

  await Appointment.update({status : "confirmed"},{where : {id : id}})
  .then((result)=>{
      res.send({payment : true,id : id})
  }).catch(err=>{
...
app.post('doctor/approve/appointment/:id' ,async (req,res) => {
  await Appointment.findOne({where: {id:req.params.id}}).then(appointment => {
    ...
    appointment.update({status:'approved'},{where:{id:req.params.id}})
    ///need to join the room
    io.sockets.in(id).emit('approved',appointment)
...
io.on('connection', socket => {
  ...
  socket.on('appointment-approve',room => {
    

      Appointment.update({status:'approved'},{where:{id:room}}).then((updated)=>{
  
        const appointmentRoom = `room no ${room}`
        socket.join(appointmentRoom)
        console.log(socket.rooms)
        socket.emit("approved",room)//patient
  ...
  socket.on("join room",(id)=>{
    ...
    const appointmentRoom = `room no ${id}`
    // console.log("joined")
    socket.join(appointmentRoom)
    console.log(socket.rooms)
  ...
  socket.on("doctorHasApproved",(appointment)=>{

      // socket.to(appointment.id).emit("doctorapproved",appointment,socket.rooms)
      
      socket.to(`room no ${appointment.id}`).emit("doctorapproved",appointment)
      // io.emit("doctorapproved",appointment)


  })
  socket.on("payment done",id=>{
    socket.to(`room no ${id}`).emit("payment succeeded",id)
  ...
  socket.on("book appointment",async (pEmail,dEmail)=>{
    console.log(dEmail)
    let pid,did,patient
    await  User.findOne({where:{email: pEmail}}).then(p => {pid=p.id;
      patient=p
      console.log('pid is',pid)})
    await  Doctor.findOne({where:{email:dEmail}}).then(doctor => {did=doctor.id
      console.log('did,fees are',did)})
       await Appointment.create({doctorId:did,patientId:pid,status:"pending",language:"hindi"}).then(appointment => {
        ...
        // console.log(`patient room is ${roomName}`)
        ...
        // console.log("successfully joined ",appointment.id)
        // io.emit("appointment",appointment)
        io.emit("appointment-created",dEmail,newAppointment)
